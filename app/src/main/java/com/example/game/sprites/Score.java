...
public class Score implements Sprite {
    ...
    public void draw(Canvas canvas) {
        if(!collision) {
            ...
            canvas.drawBitmap(bmpScore, screenWidth / 4 - bmpScore.getWidth() / 2, 3 * screenHeight / 6 - zero.getHeight() - bmpScore.getHeight(), null);
            for (int i = 0; i < currentDigits.size(); i++) {
                int x = screenWidth / 4 - currentDigits.size() * zero.getWidth() +zero.getWidth() * i;
                canvas.drawBitmap(currentDigits.get(i), x, 3 * screenHeight / 6, null);
            }

            canvas.drawBitmap(bmpBest, 3 * screenWidth / 4 - bmpBest.getWidth() / 2, 3 * screenHeight / 6 - zero.getHeight() - bmpBest.getHeight(), null);
            for (int i = 0; i < topDigits.size(); i++) {
                int x = 3 * screenWidth / 4 - topDigits.size() * zero.getWidth() +zero.getWidth() * i;
                canvas.drawBitmap(topDigits.get(i), x, 3 * screenHeight / 6, null);
            }
        }
    }
